...
kar = pd.read_excel("dataset\pakistan_aqi_weather.xlsx", sheet_name="Karachi")
kar['time'] = pd.to_datetime(kar['time'])

# Establish categorical timeline horizons
kar['Date'] = kar['time'].dt.date
kar['Month'] = kar['time'].dt.to_period('M').astype(str)
kar['Quarter'] = kar['time'].dt.to_period('Q').astype(str)
kar['Year'] = kar['time'].dt.year
...
daily_profile = compute_profile_sheet(kar, 'Date', 'daily highest', 'daily lowest')
monthly_profile = compute_profile_sheet(kar, 'Month', 'monthly highest', 'monthly lowest')
quarterly_profile = compute_profile_sheet(kar, 'Quarter', 'quarterly highest', 'quarterly lowest')
yearly_profile = compute_profile_sheet(kar, 'Year', 'yearly highest', 'yearly lowest')

# 3. Write raw profiles out to multi-tab excel sheet
excel_filename = "dataset\karachi_aggregation_report.xlsx"
